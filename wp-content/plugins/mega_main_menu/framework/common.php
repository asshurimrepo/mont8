	...
	class mm_common {
		...
		public static function get_url_content( $url ) {
			if ( function_exists( 'curl_init' ) ) {
				...
				if ( $curl_info[ 'http_code' ] == ( 400 || 302 || 303 ) ) {
					$out = false;
				} else {
					$out = curl_exec( $ch );
				}
			...
			} elseif ( ( ini_get( 'allow_url_fopen' ) && function_exists('fopen') && function_exists('fread') ) || ( is_readable( $url ) ) ) {
				$handle = fopen( $url, "rb" );
				...
				foreach ( $post_types as $post_types_key => $single_post_type ) {
					...
					foreach ( $tax[ $single_post_type ] as $tax_key => $tax_value ) {
						...
						foreach ( $tax[ $single_post_type ][ $tax_value ] as $array_key => $array_val ) {
							$single_post_type = str_replace( 'cwa', 'post', $single_post_type ); // fix for the - http://www.protalent.tv/
