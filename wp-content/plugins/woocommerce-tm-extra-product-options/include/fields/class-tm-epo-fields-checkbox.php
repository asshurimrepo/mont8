	...
	public function display_field_pre( $element=array(), $args=array() ) {
		...
		if (!empty($this->items_per_row)){
			...
			if ($this->items_per_row=="auto"){
				...
				$this->css_string=".element_".$args['element_counter'].$args["form_prefix"]." li{float:left !important;width:auto !important;}";
			}else{
				$this->items_per_row=(float) $element['items_per_row'];
				$this->_percent=(float) (100/$this->items_per_row);
				$this->css_string=".element_".$args['element_counter'].$args["form_prefix"]." li{float:left !important;width:".$this->_percent."% !important;}";	
			}
