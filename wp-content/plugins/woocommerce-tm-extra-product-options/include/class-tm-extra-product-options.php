...
// Direct access security
	...
	final class TM_Extra_Product_Options {
		...
		public function get_plugin_settings()
		{
			...
			if ( ! empty( $this->tm_epo_css_selected_border ) )
			{

				$css_string = '.tm-extra-product-options input.use_images:checked + label .radio_image, .tm-extra-product-options input.use_images:checked + label .checkbox_image{';

				switch ( $this->tm_epo_css_selected_border )
				{

					case 'round':

						$css_string .= '-webkit-border-radius: 100%;-moz-border-radius: 100%;border-radius: 100%;border-width: 1px;-webkit-box-shadow: none;-moz-box-shadow: none;box-shadow: none;';

						break;

					case 'square':

						$css_string .= '-webkit-border-radius: 0;-moz-border-radius: 0;border-radius: 0;-webkit-box-shadow: none;-moz-box-shadow: none;box-shadow: none;border-width: 2px;';

						break;

					case 'shadow':

						$css_string .= '-webkit-box-shadow: 0 1px 4px 0 rgba(0, 0, 0, 0.5);-moz-box-shadow: 0 1px 4px 0 rgba(0, 0, 0, 0.5);box-shadow: 0 1px 4px 0 rgba(0, 0, 0, 0.5);    border-width: 1px;';

						break;

					case 'thinline':

						$css_string .= 'border-width: 1px;-webkit-border-radius: 0;-moz-border-radius: 0;border-radius: 0;-webkit-box-shadow: none;-moz-box-shadow: none;box-shadow: none;';
		...
		public function add_plugin_actions()
		{
			...
			add_action( 'woocommerce_tm_custom_price_fields_enqueue_scripts', array(
				$this,
				'custom_frontend_scripts'
			) );
			...
			add_filter( 'woocommerce_tm_epo_option_price', array( $this, 'tm_epo_option_price' ), 10, 1 );
			...
			add_filter( 'woocommerce_order_item_display_meta_value', array(
				$this,
				'tm_order_item_display_meta_value'
			), 10, 1 );
			...
			add_action( 'tm_woocommerce_checkout_after_row', array(
				$this,
				'tm_woocommerce_checkout_after_row'
			), 10, 4 );

			add_action( 'tm_woocommerce_order_after_row', array( $this, 'tm_woocommerce_order_after_row' ), 10, 3 );

			add_action( 'tm_woocommerce_email_after_row', array( $this, 'tm_woocommerce_email_after_row' ), 10, 4 );


			/* Display fields on admin Order */

			add_action( 'woocommerce_order_item_' . 'line_item' . '_html', array(
				$this,
				'tm_woocommerce_order_item_line_item_html'
			), 10, 2 );
			...
			add_filter( 'woocommerce_add_to_cart_redirect', array(
				$this,
				'tm_woocommerce_add_to_cart_redirect'
			), 9999, 1 );

			add_filter( 'woocommerce_quantity_input_args', array(
				$this,
				'tm_woocommerce_quantity_input_args'
			), 9999, 2 );
		...
		public function add_compatibility_actions()
		{
			...
			add_filter( 'woocommerce_subscriptions_product_sign_up_fee', array(
				$this,
				'tm_subscriptions_product_sign_up_fee'
			), 10, 2 );
			...
			add_filter( 'woocommerce_composite_button_behaviour', array(
				&$this,
				'tm_woocommerce_composite_button_behaviour'
			), 50, 2 );
			...
			if ( $this->tm_epo_dpd_enable == "no" )
			{
				...
				add_action( 'woocommerce_cart_loaded_from_session', array(
					$this,
					'cart_loaded_from_session_99999'
				), 99999 );
					...
					foreach ( $priorities as $pid => $field )
					{
						...
						if ( isset( $field['sections'] ) && is_array( $field['sections'] ) )
						{
							...
							foreach ( $field['sections'] as $section_id => $section )
							{
								...
								$section['elements'] = (array) $section['elements'];
				...
				foreach ( $epos as $key => $epo )
				{
					...
					if ( $epo )
					{
						...
						else
						{
							...
							$html .= '<tr class="' . esc_attr( apply_filters( 'woocommerce_order_item_class', 'order_item', $item, $order ) ) . '">';

							$html .= '<td style="text-align:left; vertical-align:middle; border: 1px solid #eee; word-wrap:break-word;">' . $epo['name'] . '<div>' . $epo['value'] . '</div></td>';

							$html .= '<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;">' . ( $epo['quantity'] * (float) $item['item_meta']['_qty'][0] ) . '</td>';

							$html .= '<td style="text-align:left; vertical-align:middle; border: 1px solid #eee;">' .

							         ( ( ! empty( $item['item_meta']['tm_has_dpd'] ) ) ? '' : ( wc_price( (float) $epo['quantity'] * (float) $epo['price'] * (float) $item['item_meta']['_qty'][0] ) ) )
		...
		public function tm_woocommerce_order_after_row( $item_id, $item, $order )
		{
			...
			if ( $has_epo )
			{
				...
				foreach ( $epos as $key => $epo )
				{
					...
					if ( $epo )
					{
						...
						$html .= '<td class="product-name">' . $epo['name'] . ' <strong class="product-quantity">' . sprintf( '&times; %s', ( $epo['quantity'] * (float) $item['item_meta']['_qty'][0] ) ) . '</strong>' . '<div>' . $epo['value'] . '</div></td>';
					...
					foreach ( $epos as $key => $epo )
					{
						...
						if ( $epo )
						{
							...
							if ( empty( $item['item_meta']['tm_has_dpd'] ) )
							{
								...
								$html .= '<td class="tm-c item_cost" width="1%"><div class="view">' . wc_price( $epo['price'] ) . '</div></td>';

								$html .= '<td class="tm-c tm_quantity" width="1%"><div class="view">' . ( $epo['quantity'] * (float) $item['item_meta']['_qty'][0] ) . '</div></td>';

								$html .= '<td class="tm-c line_cost" width="1%"><div class="view"><span class="amount">' . wc_price( (float) $epo['quantity'] * (float) $epo['price'] * (float) $item['item_meta']['_qty'][0] ) . '</span></div></td>';
...
//			return $price;
		...
		public function init_settings_pre()
		{
			...
			if ( ( $product && is_object( $product ) && property_exists( $product, 'post' ) && property_exists( $product->post, 'post_type' ) && ( in_array( $product->post->post_type, array(
						'product',
						'product_variation'
					) ) ) ) || $postid == 0
			)
		...
		public function init_settings()
		{
			...
			if ( class_exists( 'WOOCS' ) )
			{
				...
				remove_filter( 'woocommerce_order_amount_total', array(
					$WOOCS,
			...
			if ( $this->tm_epo_cart_field_display == "link" )
			{
				...
				foreach ( $link_data as $link )
				{
					...
					$data .= '<div class="row tm-cart-row">'
					         ...
					         . '<div class="cell col-1 cpf-total-price">' . $link['price'] . '</div>'
		...
		public function calculate_price( $element, $key, $attribute, $per_product_pricing, $cpf_product_price, $variation_id )
		{
			...
			if ( $per_product_pricing )
			{
				...
				if ( ! isset( $element['price_rules'][ $key ] ) )
				{// field price rule

					if ( $variation_id && isset( $element['price_rules'][0][ $variation_id ] ) )
					{// general variation rule

						$_price = $element['price_rules'][0][ $variation_id ];

					}
					elseif ( isset( $element['price_rules'][0][0] ) )
					{// general rule
				...
				else
				{
					...
					if ( $variation_id && isset( $element['price_rules'][ $key ][ $variation_id ] ) )
					{// field price rule

						$_price = $element['price_rules'][ $key ][ $variation_id ];

					}
					elseif ( isset( $element['price_rules'][ $key ][0] ) )
					{// general field variation rule

						$_price = $element['price_rules'][ $key ][0];

					}
					elseif ( $variation_id && isset( $element['price_rules'][0][ $variation_id ] ) )
					{// general variation rule
				...
				if ( ! isset( $element['price_rules_type'][ $key ] ) )
				{// field price rule

					if ( $variation_id && isset( $element['price_rules_type'][0][ $variation_id ] ) )
					{// general variation rule

						$_price_type = $element['price_rules_type'][0][ $variation_id ];

					}
					elseif ( isset( $element['price_rules_type'][0][0] ) )
					{// general rule
				...
				else
				{
					...
					if ( $variation_id && isset( $element['price_rules_type'][ $key ][ $variation_id ] ) )
					{// field price rule

						$_price_type = $element['price_rules_type'][ $key ][ $variation_id ];

					}
					elseif ( isset( $element['price_rules_type'][ $key ][0] ) )
					{// general field variation rule

						$_price_type = $element['price_rules_type'][ $key ][0];

					}
					elseif ( $variation_id && isset( $element['price_rules_type'][0][ $variation_id ] ) )
					{// general variation rule
				...
				$_price = wc_format_decimal( $_price, "", true );
										...
										if ( $_original_terms )
										{
											...
											foreach ( $_original_terms as $term )
											{
												...
												if ( $has_term )
												{
													...
													$args['options'] .= '<option ' . ( isset( $_POST[ 'tmcp_' . $name_inc . $form_prefix ] ) ? selected( $_POST[ 'tmcp_' . $name_inc . $form_prefix ], esc_attr( sanitize_title( $term->slug ) ), 0 ) : "" ) . ' value="' . sanitize_title( $term->slug ) . '" data-price="" data-asshurim="" data-rules="' . ( isset( $field['rules_filtered'][ $term->slug ] ) ? esc_html( json_encode( ( $field['rules_filtered'][ $term->slug ] ) ) ) : '' ) . '" data-rulestype="' . ( isset( $field['rules_type'][ $term->slug ] ) ? esc_html( json_encode( ( $field['rules_type'][ $term->slug ] ) ) ) : '' ) . '">' . wptexturize( $wpml_term->name ) . '</option>';
								...
								switch ( $field['type'] )
								{
									...
									case "select":
										...
										foreach ( $options as $k => $option )
										{
											...
											$args['options'] .= '<option ' . ( isset( $_POST[ 'tmcp_' . $name_inc . $form_prefix ] ) ? selected( $_POST[ 'tmcp_' . $name_inc . $form_prefix ], esc_attr( sanitize_title( $option ) ), 0 ) : "" ) . ' value="' . esc_attr( sanitize_title( $option ) ) . '" data-price="" data-asshuri="" data-rules="' . ( isset( $field['rules_filtered'][ esc_attr( sanitize_title( $option ) ) ] ) ? esc_html( json_encode( ( $field['rules_filtered'][ esc_attr( sanitize_title( $option ) ) ] ) ) ) : '' ) . '" data-rulestype="' . ( isset( $field['rules_type'][ esc_attr( sanitize_title( $option ) ) ] ) ? esc_html( json_encode( ( $field['rules_type'][ esc_attr( sanitize_title( $option ) ) ] ) ) ) : '' ) . '">' . wptexturize( apply_filters( 'woocommerce_tm_epo_option_name', isset( $wpml_options[ $k ] ) ? $wpml_options[ $k ] : $option ) ) . '</option>';
		...
		public function custom_frontend_scripts()
		{
			...
			wp_enqueue_script( 'tm-epo', $this->plugin_url . '/assets/js/tm-epo.js', array(
				'jquery',
		...
		public function get_RP_WCDPD( $product, $field_price = null, $cart_item_key = null )
		{
			...
			if ( class_exists( 'RP_WCDPD' ) && class_exists( 'RP_WCDPD_Pricing' ) && ! empty( $GLOBALS['RP_WCDPD'] ) )
			{
				...
				if ( is_array( $selected_rule ) )
				{
					...
					if ( $selected_rule['method'] == 'quantity' && isset( $selected_rule['pricing'] ) && in_array( $selected_rule['quantities_based_on'], array(
							'exclusive_product',
							'exclusive_variation',
							'exclusive_configuration'
						) )
					)
					{
		...
		public function get_product_tm_epos( $post_id = 0 )
		{
			...
			$_all_categories = TM_EPO_WPML()->get_terms( null, 'product_cat', array(
				'fields'     => "ids",
				'hide_empty' => false
			) );
						...
						if ( is_array( $builder ) && count( $builder ) > 0 && isset( $builder['element_type'] ) && is_array( $builder['element_type'] ) && count( $builder['element_type'] ) > 0 )
						{
							...
							for ( $_s = 0; $_s < count( $_sections ); $_s ++ )
							{
								...
								$global_epos[ $priority ][ $tmcp_id ]['sections'][ $_s ] = array(
									...
									'sections_clogic'      => $this->get_builder_element( 'sections_clogic', $builder, $current_builder, $_s, array_fill( 0, count( $_sections ), false ), $wpml_section_fields ),
									'sections_logic'       => $this->get_builder_element( 'sections_logic', $builder, $current_builder, $_s, array_fill( 0, count( $_sections ), "" ), $wpml_section_fields ),
									'sections_class'       => $this->get_builder_element( 'sections_class', $builder, $current_builder, $_s, array_fill( 0, count( $_sections ), "" ), $wpml_section_fields ),
									'sections_type'        => $this->get_builder_element( 'sections_type', $builder, $current_builder, $_s, array_fill( 0, count( $_sections ), "" ), $wpml_section_fields ),
									...
									if ( isset( $_elements[ $k0 ] ) && isset( $this->tm_original_builder_elements[ $_elements[ $k0 ] ] ) )
									{
										...
										if ( $this->tm_original_builder_elements[ $_elements[ $k0 ] ] )
										{
											...
											if ( $this->tm_original_builder_elements[ $_elements[ $k0 ] ]["type"] == "single" )
											{
												...
												$_regular_price = array( array( wc_format_decimal( $_price, "", true ) ) );
												...
												$_regular_price_filtered = array( array( wc_format_decimal( $_price, "", true ) ) );
...
//													var_dump($_prices);
													...
													foreach ( $_prices as $_n => $_price )
													{
														...
														$_regular_price[ esc_attr( ( $_values[ $_n ] ) ) . "_" . $_n ] = array( wc_format_decimal( $_price, "", true ) );
														...
														$_regular_price_filtered[ esc_attr( ( $_values[ $_n ] ) ) . "_" . $_n ] = array( wc_format_decimal( $_price, "", true ) );
					...
					if ( isset( $section['elements'] ) && is_array( $section['elements'] ) )
					{
						...
						foreach ( $section['elements'] as $element )
						{
							...
							$args = array(
								...
								'class_id'             => "tm-element-ul-" . $element['type'] . " element_" . $element_counter . $form_prefix,
								// this goes on ul
									...
									if ( $this->tm_builder_elements[ $element['type'] ]["type"] == "single" || $this->tm_builder_elements[ $element['type'] ]["type"] == "multiplesingle" )
									{
										...
										elseif ( ! empty( $_GET['tm_cart_item_key'] ) && isset( $_GET['_wpnonce'] ) && wp_verify_nonce( $_GET['_wpnonce'], 'tm-edit' ) )
										{
											...
											add_filter( 'woocommerce_product_single_add_to_cart_text', array(
												$this,
												'tm_woocommerce_product_single_add_to_cart_text'
											), 9999 );
