...
final class TM_EPO_ADMIN_Global_base {
        ...
        switch ( $column ) {
            ...
            case 'tm_icl_translations':
                ...
                if (empty($main_post_id)){
                    $main_post_id=get_post_meta( $post_id , 'tm_meta_parent_post_id' , true );
                }
    ...
    public function tm_product_search_meta_box($post){
        ...
        <?php if(version_compare( get_option( 'woocommerce_db_version' ), '2.3', '<' )){?>
                <select <?php echo $disabled;?>id="tm_product_ids" 
                    ...
                    multiple="multiple" 
                    data-placeholder="<?php _e( 'Search for a product&hellip;', TM_EPO_TRANSLATION ); ?>">
                    <?php 
    ...
    public function tm_admin_init(){
        ...
        if ( ! $post_type_object ){
            wp_die( __( 'WooCommerce is not enabled!' , TM_EPO_TRANSLATION) );
        }
    ...
    public function register_admin_scripts($override=0) {
        ...
        $params = array(
            ...
            'i18n_no_variations'    => esc_js( __( 'There are no saved variations yet.', TM_EPO_TRANSLATION ) ),
            'i18n_cancel'           => __( 'Cancel', TM_EPO_TRANSLATION ),
            'edit_settings'         => __( 'Edit settings', TM_EPO_TRANSLATION ),
            'element_uniqid'         => __( 'Element id', TM_EPO_TRANSLATION ),
    ...
    public function tm_save_postdata( $post_id,$post_object ) {
        ...
        // WPML fields
        if (TM_EPO_WPML()->is_active()){
            if ( isset($_POST['tm_meta_parent_post_id']) ){
                $old_data = get_post_meta($post_id, 'tm_meta_parent_post_id',true);
                $this->tm_save_meta($post_id, $_POST['tm_meta_parent_post_id'], $old_data, 'tm_meta_parent_post_id');            
            }else{
                //$old_data = get_post_meta($post_id, 'tm_meta_parent_post_id',true);
                //$this->tm_save_meta($post_id, 0, $old_data, 'tm_meta_parent_post_id');
            }
            //if ( isset($_POST[TM_EPO_WPML_LANG_META]) ){
            if ( isset($_POST[TM_EPO_WPML_LANG_META]) && !empty($_POST[TM_EPO_WPML_LANG_META])){
                ...
                //$old_data = get_post_meta($post_id, 'tm_meta_lang',true);
                $old_data = get_post_meta($post_id, TM_EPO_WPML_LANG_META,true);
                //$this->tm_save_meta($post_id, 0, $old_data, 'tm_meta_lang');
