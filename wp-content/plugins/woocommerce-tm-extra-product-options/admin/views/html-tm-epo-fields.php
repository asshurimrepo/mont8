...
<div class="wrap">
	...
	<h2><?php _e( 'TM Global Extra Product Options', TM_EPO_TRANSLATION );
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		if (isset($tm_nonce)){
			$messages[] = '<a href="' . esc_url( wp_nonce_url( "edit.php?post_type=product&page=tm-global-epo&doaction=undo&action=untrash&ids=$ids".$tm_bulk, $tm_nonce ) ) . '">' . __('Undo', TM_EPO_TRANSLATION) . '</a>';
		}
	}
}
...
<input type="hidden" name="page" class="page_page" value="tm-global-epo" />
