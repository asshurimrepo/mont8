...
$_field_tmcp_variation .= '<select class="tmcp-variation" name="tmcp_variation' .  '[' . $loop . ']"><option value="0">' . __( 'Any', TM_EPO_TRANSLATION ) . ' ' .  '&hellip;</option>';
...
foreach ( $parent_data['attributes'] as $attribute ) {
    ...
    $_field_attribute .= '<select data-tm-attr="'.esc_attr(sanitize_title( $attribute['name'] )) .'" class="tmcp_att tmcp_attribute_'.sanitize_title( $attribute['name'] ) .'" name="attribute_' . sanitize_title( $attribute['name'] ) . '[' . $loop . ']"><option value="0">' . __( 'Any', TM_EPO_TRANSLATION ) . ' ' . esc_html( wc_attribute_label( $attribute['name'] ) ) . '&hellip;</option>';
    ...
    if ( $attribute['is_taxonomy'] ) {
        $all_terms = get_terms( $attribute['name'], 'orderby=name&hide_empty=0' );
            ...
            foreach ( $all_terms as $term ) {
                ...
                if ($has_term ){
                    $_field_attribute .= '<option value="' . esc_attr( $term->slug ) . '" >' . apply_filters( 'woocommerce_tm_epo_option_name', esc_html( $term->name ) ) . '</option>';
                }
            }
        }        
    } else {
        ...
        foreach ( $options as $option ) {
            $_field_attribute .= '<option value="' . esc_attr( sanitize_title( $option ) ) . '">' . esc_html( apply_filters( 'woocommerce_tm_epo_option_name', $option ) ) . '</option>';
        }
    }
...
<div data-epo-attr="<?php echo esc_attr( sanitize_title( $tmcp_attribute_selected_value ) ); ?>" class="woocommerce_tm_epo wc-metabox closed">
    <h3>
        ...
        <div class="handlediv" title="<?php _e( 'Click to toggle', TM_EPO_TRANSLATION ); ?>"></div>
            ...
            <span class="tm-type-label"><?php _e( 'Type:', TM_EPO_TRANSLATION ); ?></span>
            <select class="tm-type" name="tmcp_type[<?php echo $loop; ?>]">
                    <option <?php selected(  $tmcp_type_selected_value , 'radio' ) ?> value="radio"><?php _e( 'Radio buttons', TM_EPO_TRANSLATION ); ?></option>
                    <option <?php selected(  $tmcp_type_selected_value , 'checkbox' ) ?> value="checkbox"><?php _e( 'Checkbox', TM_EPO_TRANSLATION ); ?></option>
                    <option <?php selected(  $tmcp_type_selected_value , 'select' ) ?> value="select"><?php _e( 'Select', TM_EPO_TRANSLATION ); ?></option>
            </select>
            <span class="tm-options">
                <span class="tm-hide-price">
                    <label><input type="checkbox" class="checkbox" name="tmcp_hide_price[<?php echo $loop; ?>]" <?php checked( $tmcp_hide_price, 1 ); ?> value="1" /> <?php _e( 'Hide price', TM_EPO_TRANSLATION ); ?></label>
                </span>
                <span class="tm-required">
                    <label><input type="checkbox" class="checkbox" name="tmcp_required[<?php echo $loop; ?>]" <?php checked( $tmcp_required, 1 ); ?> value="1" /> <?php _e( 'Required', TM_EPO_TRANSLATION ); ?></label>
                </span>
                <span class="tm-enabled">
                    <label><input type="checkbox" class="checkbox" name="tmcp_enabled[<?php echo $loop; ?>]" <?php checked( $tmcp_post_status, 'publish' ); ?> /> <?php _e( 'Enabled', TM_EPO_TRANSLATION ); ?></label>
                </span>
            </span>
                ...
                <td class="data" rowspan="2">
                    <table cellspacing="0" cellpadding="0" class="data_table">
                        <tr class="tmcp_choices">
                            <td>
                                ...
                                if ($tmcp_type_selected_value=="checkbox"){
                                    echo '<span class="tm-hide-price"><label>'.__( 'Limit selection', TM_EPO_TRANSLATION ).': <input type="text" name="tmcp_limit['.$loop.']" value="'.$tmcp_limit.'" /></label></span>';
                                }else{
                        ...
                        <tr class="tmcp_variation show_if_variable">
                            <td>
                                <?php 
                                _e( 'Variation:', TM_EPO_TRANSLATION );
                        ...
                        <tr class="tmcp_pricing">
                            <td>
                                <label><?php echo __( 'Price:', TM_EPO_TRANSLATION ) . ' (' . get_woocommerce_currency_symbol() . ')'; ?></label>
                                <?php
                                ...
                                ?>
                                <input type="text" size="5" name="tmcp_regular_price[<?php echo $loop; ?>][<?php echo esc_attr( $key_attribute ); ?>][<?php echo $key_variation; ?>]" value="<?php echo esc_attr( $price ); ?>" class="wc_input_price tmcp-price-input tmcp-price-input-variation-<?php echo $key_variation; ?>" data-price-input-attribute="<?php echo esc_attr( $key_attribute ); ?>" placeholder="<?php _e( 'Custom price (required)', TM_EPO_TRANSLATION ); ?>" />
                                <select class="tmcp-price-input-type tmcp-price-input-variation-<?php echo $key_variation; ?>" data-price-input-attribute="<?php echo esc_attr( $key_attribute ); ?>" name="tmcp_regular_price_type[<?php echo $loop; ?>][<?php echo esc_attr( $key_attribute ); ?>][<?php echo $key_variation; ?>]">
                                    <option <?php selected(  $_regular_price_type[$key_attribute][$key_variation] , '' ) ?> value=""><?php _e( 'Fixed amount', TM_EPO_TRANSLATION ); ?></option>
                                    <option <?php selected(  $_regular_price_type[$key_attribute][$key_variation] , 'percent' ) ?> value="percent"><?php _e( 'Percent of the orignal price', TM_EPO_TRANSLATION ); ?></option>
                                </select>
                                    <?php
                                        }
                                    }
                                ...
                                ?>
                                <input type="text" size="5" name="tmcp_regular_price[<?php echo $loop; ?>][0][0]" value="" class="wc_input_price tmcp-price-input tmcp-price-input-variation-0 tmcp-price-input-attribute-0" data-price-input-attribute="0" placeholder="<?php _e( 'Custom price', TM_EPO_TRANSLATION ); ?>" />
                                <select class="tmcp-price-input-type tmcp-price-input-variation-0 tmcp-price-input-attribute-0" data-price-input-attribute="0" name="tmcp_regular_price_type[<?php echo $loop; ?>][0][0]">
                                    <option value=""><?php _e( 'Fixed amount', TM_EPO_TRANSLATION ); ?></option>
                                    <option value="percent"><?php _e( 'Percent of the orignal price', TM_EPO_TRANSLATION ); ?></option>
                                </select>
                                <?php
                                }
                                ?>
...
<div data-epo-attr="<?php echo esc_attr( sanitize_title( $tmcp_attribute_selected_value ) ); ?>" class="woocommerce_tm_epo wc-metabox closed">
    <h3>
        ...
        <div class="handlediv" title="<?php _e( 'Click to toggle', TM_EPO_TRANSLATION ); ?>"></div>
            ...
            <input type="hidden" value="<?php echo esc_attr( sanitize_title( $tmcp_attribute_selected_value ) ); ?>" class="tmcp_attribute" name="tmcp_attribute[<?php echo $loop; ?>]">
            <?php _e( 'Attributes missing. Please DELETE this extra option:', TM_EPO_TRANSLATION );
            ?>
                ...
                <td class="data" rowspan="2">
                    <table cellspacing="0" cellpadding="0" class="data_table">
                        ...
                        <tr class="tmcp_attribute">
                            <td>
                                <?php 
                                _e( 'Attributes missing. Please DELETE this extra option:', TM_EPO_TRANSLATION );
