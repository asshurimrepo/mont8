...
<div id="woocs_order_metabox">
    <strong><?php _e('Order curency', 'woocommerce-currency-switcher') ?></strong>: 
    ...
    <strong><?php _e('Base curency', 'woocommerce-currency-switcher') ?></strong>: <?php echo $base_currency ?><br />
    <strong><?php _e('Order curency rate', 'woocommerce-currency-switcher') ?></strong>: <?php echo $rate ?>&nbsp;<span class="tips" data-tip="<?php _e('Currency rate when the customer paid ', 'woocommerce-currency-switcher') ?>">[?]</span><br />
    <strong><?php _e('Total amount', 'woocommerce-currency-switcher') ?></strong>: <?php $_REQUEST['no_woocs_order_amount_total']=1 ;echo trim(number_format($order->get_total() * $rate, 2) . ' ' . $currency) ?><br />
    <hr />
    <a href="javascript:woocs_change_order_data();void(0);" class="button woocs_change_order_curr_button"><?php _e('change order woocs currency', 'woocommerce-currency-switcher') ?></a>
