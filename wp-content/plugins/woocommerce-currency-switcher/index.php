...
final class WOOCS
{
    ...
    public $the_plugin_version = '2.0.8';
    ...
    public function init()
    {
        ...
        add_filter('woocommerce_email_actions', array($this, 'woocommerce_email_actions'), 999);
    ...
    public function print_plugin_options()
    {
        if (isset($_POST['woocs_name']) AND ! empty($_POST['woocs_name']))
        {
            ...
            update_option('woocs_currencies_rate_auto_update', $_POST['woocs_currencies_rate_auto_update']);
        ...
        //wp_enqueue_style('jquery-ui', WOOCS_LINK . 'css/jquery-ui-1.8.23.custom.css');
            ...
            if (isset($currencies[$_REQUEST['woocs_in_order_currency']]))
            {
                ...
                if (is_object($post))
                {
                    ...
                    } else
                    {
                        ...
                        $price = number_format($order->get_total() * $rate, 2, $this->decimal_sep, '');
                    }
                }
            }
        } else
        {
            if ($this->current_currency != $this->default_currency)
            {
                ...
                if (is_ajax())
                {
                    ...
                    {
                        if (
                                $_REQUEST['action'] == 'woocommerce_checkout'
                                OR $_REQUEST['action'] == 'woocommerce_update_order_review'
                                OR $_REQUEST['action'] == 'woocommerce_get_refreshed_fragments'
                                OR $_REQUEST['action'] == 'woocommerce_update_shipping_method'
                        )
                        {
                            if ($this->is_multiple_allowed)
                            {
    ...
    public function woocommerce_order_amount_total_shipping($order_shipping)
    {
        ...
        $order_shipping = number_format($order_shipping, 2, $this->decimal_sep, '');
        if ($this->current_currency == 'JPY')
        {
            $order_shipping = intval($order_shipping);
        }
        return $order_shipping;
    }

    //for paypal only
    public function woocommerce_order_amount_total_tax($amount)
    {
        //woocommerce\includes\gateways\paypal\class-wc-gateway-paypal.php #429
        //woocommerce\includes\class-wc-order.php #523 get_total_tax - maybe the same will need
        static $done = 0;
        ...
        if ($this->is_multiple_allowed AND ! $done AND is_ajax() AND isset($_REQUEST['payment_method']))
        {
            ...
            {
                $currencies = $this->get_currencies();
                $amount = $amount * $currencies[$this->current_currency]['rate'];
                //$done = 1;
                //$amount = number_format($amount, 2, $this->decimal_sep, $this->thousands_sep);
                $amount = number_format($amount, 2, $this->decimal_sep, '');
            }
        }
    ...
    public function woocommerce_order_amount_total($order_total)
    {
        ...
        //FOR PAYPAL FIXING
        //Validation error: PayPal amounts do not match (gross 56.28).
        //Order status changed from Pending Payment to On Hold.
        static $mc_gross_done = 0;
        if (isset($_REQUEST['pending_reason'])
                AND isset($_REQUEST['mc_gross'])
                AND isset($_REQUEST['pending_reason'])
                AND $mc_gross_done == 0
                AND isset($_REQUEST['payment_status']))
        {
            //$this->log($_REQUEST['mc_gross']);
            $mc_gross_done++;
            return $_REQUEST['mc_gross'];
        }
        ...
        if ($this->is_multiple_allowed AND ! $done AND ! isset($_REQUEST['no_woocs_order_amount_total']))
        {
            ...
            if (isset($_REQUEST['woocs_order_emails_is_sending']))
            {
                //$this->log($_REQUEST['woocs_order_emails_is_sending']);
                if (@$_REQUEST['payment_method'] != 'paypal')
                {
                    $done = 1; //this need for emails only if paid NOT by paypal!!!
                }
            }
            ...
            if ($this->current_currency == 'JPY')
            {
                $order_total = number_format($order_total, 0, $this->decimal_sep, '');
            }
        }
    ...
    public function woocommerce_before_resend_order_emails($order)
    {
        ...
        $_REQUEST['before_resend_order_emails_was'] = 1;
    }
