<?php
		...
		switch ( $type )
		{
			case 'text':
				...
				<input type="text" name="<?php echo $name; ?>" id="<?php echo $name; ?>"
				       value="<?php echo esc_attr( $value ); ?>" class="<?php echo $class; ?>"
			...
			case 'textarea':
				...
				?>
				<textarea name="<?php echo $name; ?>" id="<?php echo $name; ?>" rows="<?php echo $rows; ?>"
				          class="<?php echo $class; ?>"
				          placeholder="<?php echo $placeholder; ?>"><?php echo esc_textarea( $value ); ?></textarea>
				<?php
				break;
			...
			case 'checkbox':
				...
				<label class="<?php echo $class; ?>" for="<?php echo $name; ?>">
					...
					<input name="<?php echo $name; ?>" id="<?php echo $name; ?>" value="yes"
					       type="checkbox"<?php checked( $value, 'yes' ); ?>>
					<?php echo $label; ?>
				</label>
			...
			case 'select':
				...
				<select name="<?php echo $name; ?>" id="<?php echo $name; ?>" class="<?php echo $class; ?>">
					...
					{ ?>
						<option
							value="<?php echo esc_attr( $key ); ?>"<?php selected( $value, $key ); ?>><?php echo $label; ?></option>
					<?php } ?>
				</select>
			...
			case 'number':
				...
				<input type="number" name="<?php echo $name; ?>" id="<?php echo $name; ?>"
				       value="<?php echo esc_attr( $value ); ?>" class="<?php echo $class; ?>"
				       placeholder="<?php echo $placeholder; ?>" min="<?php echo esc_attr( $min ); ?>"
				       step="<?php echo esc_attr( $step ); ?>" size="<?php echo esc_attr( $size ); ?>">
				<?php
				break;
		}
	}
	...
	function dokan_get_template_part( $slug, $name = '' )
	{
		...
		$template = locate_template( array(
			$dokan->template_path() . "{$slug}-{$name}.php",
			$dokan->template_path() . "{$slug}.php"
		) );
	...
	function dokan_get_coupon_edit_url( $coupon_id, $coupon_page = '' )
	{
		...
		$edit_url = wp_nonce_url( add_query_arg( array(
			'post'   => $coupon_id,
			'action' => 'edit',
			'view'   => 'add_coupons'
		), $coupon_page ), '_coupon_nonce', 'coupon_nonce_url' );
...
// Hook into the 'wp_before_admin_bar_render' action
	...
	function dokan_product_listing_filter()
	{
		...
		<form class="dokan-form-inline dokan-w6" method="get">
			...
			<button type="submit" name="product_listing_filter" value="ok"
			        class="dokan-btn dokan-btn-theme"><?php _e( 'Filter', 'dokan' ); ?></button>
		...
		<form method="get" class="dokan-form-inline dokan-w6">
			...
			<button type="submit" name="product_listing_search" value="ok"
			        class="dokan-btn dokan-btn-theme dokan-right"><?php _e( 'Search', 'dokan' ); ?></button>

			<?php wp_nonce_field( 'dokan_product_search', 'dokan_product_search_nonce' ); ?>

			<div class="dokan-form-group dokan-right">
				<input type="text" class="dokan-form-control" name="product_search_name" placeholder="Search Products"
				       value="<?php echo isset( $_GET['product_search_name'] ) ? $_GET['product_search_name'] : '' ?>">
			</div>
