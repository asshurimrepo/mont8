<?php
	...
	function dokan_variable_product_type_options()
	{
			...
			<div id="variable_product_options_inner">
					...
					<div id="message" class="inline woocommerce-message">
						<div class="squeezer">
							...
							<p class="submit"><a class="button-primary"
							                     href="http://docs.woothemes.com/document/product-variations/"
							                     target="_blank"><?php _e( 'Learn more', 'woocommerce' ); ?></a></p>
						</div>
					</div>
				...
				<?php else : ?>
					...
					<p class="toolbar">
						...
						<button type="button"
						        class="btn btn-sm btn-success button-primary add_variation" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Add Variation', 'dokan' ); ?></button>

						<button type="button"
						        class="btn btn-sm btn-default link_all_variations" <?php disabled( $variation_attribute_found, false ); ?>><?php _e( 'Link all variations', 'dokan' ); ?></button>
		...
		wp.media.view.settings.post = <?php echo json_encode( array(
		'param'   => 'dokan',
		'post_id' => $post->ID
	) ); // big juicy hack.
		?>;
	...
	function dokan_get_readable_seller_rating( $seller_id )
	{
		...
		<span class="seller-rating">
            <span title="<?php echo esc_attr( $text ); ?>" class="star-rating" itemtype="http://schema.org/Rating"
		...
		<span class="text"><a
				href="<?php echo dokan_get_review_url( $seller_id ); ?>"><?php printf( $long_text, $rating['rating'], $rating['count'] ); ?></a></span>
...
//				$_SESSION['send_seller_verification_email'] = true;
	...
	function dokan_exclude_child_customer_receipt( &$phpmailer )
	{
		...
		$sub_receipt  = str_replace( array(
			'{site_title}',
			'{order_date}'
		), array( wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), '' ), $sub_receipt );
		$sub_download = str_replace( array(
			'{site_title}',
			'{order_date} is complete'
		), array( wp_specialchars_decode( get_option( 'blogname' ), ENT_QUOTES ), '' ), $sub_download );
