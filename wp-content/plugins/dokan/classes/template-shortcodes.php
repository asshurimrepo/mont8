	...
	class Dokan_Template_Shortcodes {
		...
		function handle_all_submit()
		{
			...
			if ( isset( $_POST['add_product'] ) && wp_verify_nonce( $_POST['dokan_add_new_product_nonce'], 'dokan_add_new_product' ) )
			{
				...
				$is_printshop   = isset( $_POST['is_printshop'] ) ?: false;
				...
				if ( dokan_get_option( 'product_category_style', 'dokan_selling', 'single' ) == 'single' && ! $is_printshop )
				{
				...
				{
					if ( ! isset( $_POST['product_cat'] ) && empty( $_POST['product_cat'] ) && ! $is_printshop )
					{
				...
				if ( ! isset( $_POST['product_tag'] ) && empty( $_POST['product_tag'] ) && ! $is_printshop )
				{
				...
				if ( ! self::$errors )
				{
					...
					if ( $product_id )
					{
						/** set product category if printshop * */
						if($is_printshop){
							$term = get_term_by( 'slug', 'printshop', 'product_cat' );
							$_POST['product_cat'][] = $term->term_id;
						}
								...
								<div class="dokan-store-thumbnail">
									...
									<a href="<?php echo $store_url; ?>">
										<?php if ( $banner_id )
										{
											...
											?>
											<img class="dokan-store-img" src="<?php echo esc_url( $banner_url[0] ); ?>"
											     alt="<?php echo esc_attr( $store_name ); ?>">
										<?php }
										else
										{ ?>
											<img class="dokan-store-img"
											     src="<?php echo dokan_get_no_seller_image(); ?>"
											     alt="<?php _e( 'No Image', 'dokan' ); ?>">
										<?php } ?>
									</a>
									...
									<div class="dokan-store-caption">
										...
										<address>
											...
											{ ?>
												<br>
												<abbr
													title="<?php _e( 'Phone', 'dokan' ); ?>"><?php _e( 'P:', 'dokan' ); ?></abbr> <?php echo esc_html( $store_info['phone'] ); ?>
											<?php } ?>

										</address>

										<p><a class="dokan-btn dokan-btn-theme"
										      href="<?php echo $store_url; ?>"><?php _e( 'Visit Store', 'dokan' ); ?></a>
										</p>
