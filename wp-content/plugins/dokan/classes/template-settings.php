	...
	class Dokan_Template_Settings {
		...
		function insert_settings_info()
		{
			...
			elseif ( wp_verify_nonce( $_POST['_wpnonce'], 'dokan_payment_settings_nonce' ) )
			{
				...
				if ( isset( $_POST['settings']['western_union'] ) )
				{
					$wu = $_POST['settings']['western_union'];

					$dokan_settings['payment']['western_union'] = array(
						'name'    => sanitize_text_field( $wu['name'] ),
						'address' => sanitize_text_field( $wu['address'] ),
					);
				}
