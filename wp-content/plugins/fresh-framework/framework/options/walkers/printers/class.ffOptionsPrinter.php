...
/******************************************************************************/	
	...
	private function _beforeRepeatableVariableTemplateCallback( $item, $currentLevel ) {
		...
		echo '<li class="ff-repeatable-template-holder"><ul class="ff-repeatable-template '.$this->_getUlLevelClass().'" data-section-name="'.$sectionName.'" data-section-id="'.$item->getId().'" data-current-level="'.$currentLevel.'">'."\n";
	}
	...
	private function _beforeRepeatableVariableNodeCallback( $item, $index ) {
		echo '<li class="ff-repeatable-item-'.$index.' ff-repeatable-item ff-repeatable-item-closed '.$this->_getCssClass( ffOptionsPrinter::CSS_FF_REPEATABLE_ITEM ).'" data-section-id="'.$item->getId().'" data-node-id="'.$index.'">'."\n";
	}
	...
	protected function _beforeContainerCallback( $item ) {
		...
		if( $item->getType() == ffOneSection::TYPE_REPEATABLE || $item->getType() == ffOneSection::TYPE_REPEATABLE_VARIABLE ) {
			...
			echo '<ul class="ff-repeatable '.$classes.' '.$class.' '.$this->_getCssClass( ffOptionsPrinter::CSS_FF_REPEATABLE ).'" data-current-level="'.$currentLevel.'">'."\n";
		}
	}
